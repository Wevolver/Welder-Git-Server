...
def split_commit_message(commit_message):
    splits = [
        50, (commit_message.index('\n') if '\n' in commit_message else
             float("inf")), (commit_message.index('. ')
                             if '. ' in commit_message else float("inf"))
    ]

    return [commit_message[:min(splits) + 1], commit_message[min(splits) + 1:]]
